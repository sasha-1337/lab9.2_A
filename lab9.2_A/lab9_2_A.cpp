...
int main()
{
	...
	do {
		...
		case 5:
			...
			cout << " посада (0 - програміст, 1 - інженер, 2 - електрик): ";
			cin >> ispec;
...
void PrintIndexSorted(Student* p, int* I, const int N)
{
	...
	for (int i = 0; i < N; i++)
	{
		...
		case PROGRAMMER:
			cout << "| " << setw(7) << setprecision(2) << fixed
				<< p[I[i]].phisic << "| " << p[I[i]].math << setw(11) << right
				<< "|" << p[I[i]].inform << setw(13) << "|" << setprecision(2) << midMark(N, p[I[i]].phisic, p[I[i]].math, p[I[i]].inform) << " |" << endl;
			break;
		case ENGINEER:
			cout << "| " << setw(7) << setprecision(2) << fixed
				<< p[I[i]].phisic << "| " << p[I[i]].math << setw(11) << right
